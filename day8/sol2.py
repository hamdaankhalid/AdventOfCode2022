...
def move_on_grid(rows, columns, row_index: int, col_index: int):
    """
...
def calculate_sum_of_visible_trees(forrest) -> int:
...
def calculate_highest_view(forrest) -> int:
    rows, columns = forrest
    max_distance = 0
    ...
    for row_index, row in enumerate(rows):
        for col_index, col in enumerate(row):
            ...
            total_distance = distance_to_right * distance_to_left * distance_to_top * distance_to_down
